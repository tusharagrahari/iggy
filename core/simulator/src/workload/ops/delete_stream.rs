...
pub use metadata::stm::result::DeleteStreamResult as Outcome;
...
pub fn sample(
    ...
    match outcome {
        Outcome::Ok => shadow.pick_stream_name(prng).map(|name| Input { name }),
        Outcome::StreamNotFound => Some(Input {
            name: shadow.fabricate_absent_name("stream"),
        }),
    }
}
...
pub fn predicted_effect(input: &Input, outcome: Outcome) -> Effect {
    match outcome {
        Outcome::Ok => Effect::RemoveStream {
            name: input.name.clone(),
        },
        Outcome::StreamNotFound => Effect::None,
    }
}
