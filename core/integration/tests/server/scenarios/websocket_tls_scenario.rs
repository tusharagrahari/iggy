...
pub async fn run(client: &IggyClient) {
    let stream_name = "test-websocket-tls-stream";
    ...
    let topic_name = "test-websocket-tls-topic";
    client
        .create_topic(
    ...
    let mut messages = vec![
        IggyMessage::builder()
            .id(1)
            .payload(Bytes::from("Hello WebSocket TLS!"))
            .build()
            .unwrap(),
    ];
    ...
    assert_eq!(
        polled_messages.messages[0].payload.as_ref(),
        b"Hello WebSocket TLS!"
    );
