...
pub async fn run_pat_cleaner(shard: Rc<ServerShard>, stop: Receiver<()>, interval: Duration) {
    trace!(
        shard = shard.id,
...
async fn clean_expired_tokens(shard: &Rc<ServerShard>, stop: &Receiver<()>) -> Pass {
